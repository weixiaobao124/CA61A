    ...
    def vend(self):
        if self.stocks == 0:
            ...
            if self.funds < self.price:
                num = self.price - self.funds
                return 'You must add ${0} more funds.'.format(num)
                ...
                self.stocks -= 1
                num = self.funds - self.price
                self.funds = 0
                return 'Here is your {0} and ${1} change.'.format(self.product, num)
...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        return is_bst(t.branches[0]) and (t.label >= bst_min(t.branches[0]) or t.label < bst_max(t.branches[-1]))
