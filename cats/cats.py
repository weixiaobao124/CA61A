...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def func(str):
        s = split(lower(remove_punctuation(str)))
        for i in s:
            if i in topic:
                return True
        return False
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if not differences:
        return user_word
    i, target = 0, min(differences)
    if target > limit:
        return user_word
    while i < len(differences):
        if differences[i] == target:
...
def shifty_shifts(start, goal, limit):
    ...
    def dfs(a, b, u, limit):
        if len(a) == 0 or len(b) == 0:
            return u + abs(len(a) - len(b))
        if u > limit:
            return u
        elif a[0] == b[0]:
            return dfs(a[1:], b[1:], u, limit)
        else:
            return dfs(a[1:], b[1:], u + 1, limit)
    return dfs(start, goal, 0, limit)
    # END PROBLEM 6


def pawssible_patches(start, goal, limit):
    """A diff function that computes the edit distance from START to GOAL."""
    if limit == -1: # Feel free to remove or add additional cases
        return 0
        # END
...
def time_per_word(times_per_player, words):
    ...
    for item in times_per_player:
        a += [[item[i]-item[i-1] for i in range(1, len(item))]]
    return game(words, a)
    # END PROBLEM 9
